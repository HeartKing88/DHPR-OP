...
from SHUKLA.database import gmutedb as Gmute
from SHUKLA.modules.help import add_command_help
...
async def gban_user(client: Client, message: Message):
    ...
    if (await SHUKLA.gban_info(user.id)):
    ...
    for gokid in f_chats:
        try:
            ...
            er += 1
    await SHUKLA.gban_user(user.id)
    ok.append(user.id)
    msg = (
        r"**\\#GBanned_User//**"
...
async def ungban_user(client: Client, message: Message):
    ...
    try:
        if not (await SHUKLA.gban_info(user.id)):
        ...
        for good_boi in ung_chats:
            try:
                ...
                er += 1
        await SHUKLA.ungban_user(user.id)
        msg = (
            r"**\\#UnGbanned_User//**"
            f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})"
            f"\n**User ID:** `{user.id}`"
...
if ok:
 ...
 async def globals_check(client: Client, message: Message):
    ...
    if not user_id:
        return
    if (await SHUKLA.gban_info(user_id)):
        try:
